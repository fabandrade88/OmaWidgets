...
// A left pod, a right pod, the case, or a headset — drawn rather than set in a
// font, and drawn from rounded rectangles rather than from a path.
//
// A single-path version was tried and reverted. Drawing the housing and the stem
// as one shape unions them without the seam two overlapping shapes can leave
// along their join, which sounds strictly better and was not: side by side the
// two were a wash at card size, and the rectangles held up better at thirty
// pixels, where the path's thinner stem started to disappear.
//
